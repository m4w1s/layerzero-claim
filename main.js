...
async function claim(wallet, allocation) {
  ...
  const zroClaimed = await Promise.any(
    providers.map((provider) => getClaimContract(wallet.connect(provider)).zroClaimed.staticCall(wallet.address)),
  );
  ...
  for (let attempts = 4; attempts >= 0; attempts--) {
    try {
      ...
      const transaction = await Promise.any(
        providers.map((provider) => {
          return getClaimAndDonateContract(wallet.connect(provider)).donateAndClaim(
            currency,
            ethValue,
            allocation.amount,
...
function getClaimAndDonateContract(runner) {
